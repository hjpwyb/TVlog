# md/test22.py —— 最终标准 TVbox TXT 格式生成版本 (已支持直接免验证下载 httop.top/hotel.m3u)
import re
import requests
...
# -------------------- 配置 --------------------
# 直接指定的远程 m3u 链接（无需 httop_links.txt 文件）
...
# ==================== 1. 下载远程 m3u 内容 ====================
def download_remote_m3u(url: str) -> str:
    """免 SSL 验证下载 m3u 内容"""
    try:
        response = requests.get(url, timeout=30, verify=False)  # 关键：verify=False
        response.raise_for_status()
        print(f"✅ 成功下载远程 m3u 文件: {url}")
...
# 下载并可选保存原始文件
m3u_content = download_remote_m3u(REMOTE_M3U_URL)
